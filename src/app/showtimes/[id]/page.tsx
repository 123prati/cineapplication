...
export default function ShowtimeBookingPage({
  ...
  const loadData = async () => {
    try {
      const [stRes, seatsRes, userRes] = await Promise.all([
        fetch(`/api/showtimes/${showtimeId}`).then((r) => r.json()),
        fetch(`/api/showtimes/${showtimeId}/seats`).then((r) => r.json()),
        fetch("/api/auth/me").then((r) => r.json()),
      ]);

      if (!stRes.success) {
        setError(stRes.error?.message || "Failed to load showtime");
        return;
      }
      if (!seatsRes.success) {
        setError(seatsRes.error?.message || "Failed to load seat layout");
        return;
      }
  ...
  useEffect(() => {
    loadData();
    // Auto-refresh seat availability every 15s
  ...
  const handleProceedToCheckout = async () => {
    ...
    if (!currentUser) {
      ...
      router.push(`/auth/login?returnUrl=/showtimes/${showtimeId}`);
      return;
    }
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        setError(json.error?.message || "Failed to hold seats. They may have just been reserved.");
        // Refresh seats map
        await loadData();
        return;
      }
      ...
      setError(err.message || "An unexpected error occurred during reservation");
    } finally {
      setSubmitting(false);
    }
  };
                ...
                >
                  {submitting ? (
                    ...
                    <>
                      ...
                      {currentUser ? "Proceed to Checkout" : "Sign In to Reserve"}
                    </>
                  )}
                </button>
              </div>
